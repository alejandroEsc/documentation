...
1. Delete or disable the Redpanda Operator:

  You can delete the Redpanda Operator using Helm or disable it by changing the image to a nonexistent one.

  ```bash
  kubectl edit pod <operator-name> --namespace <namespace>
  ```

1. Remove the [finalizers](https://kubernetes.io/docs/concepts/overview/working-with-objects/finalizers/) from the Redpanda, HelmRelease, HelmChart, and HelmRepository custom resources:

  ```bash
  kubectl --namespace <namespace> patch redpanda <release-name> -p '{"metadata":{"finalizers":null}}' --type=merge
  kubectl --namespace <namespace> patch helmrelease <release-name> -p '{"metadata":{"finalizers":null}}' --type=merge
  kubectl --namespace <namespace> patch helmchart <release-name> -p '{"metadata":{"finalizers":null}}' --type=merge
  kubectl --namespace <namespace> patch helmrepository <release-name> -p '{"metadata":{"finalizers":null}}' --type=merge
  ```

1. Delete the resources:

  ```bash
  kubectl delete redpanda <release-name> --namespace <namespace>
  kubectl delete helmrelease <release-name> --namespace <namespace>
  kubectl delete helmchart <release-name> --namespace <namespace>
  kubectl delete helmrepository <release-name> --namespace <namespace>
  ```

After completing these steps, Redpanda Operator is no longer managing your Helm deployment.
